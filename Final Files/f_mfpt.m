...
% Computes transition probability matrix on it's own
function [Ediff,MFPT,r] = f_mfpt(P)
mask_ut = triu(true(size(P)),1);
...
MFPT(logical(eye(size(MFPT))))=r;
Ediff = mean(1./MFPT(mask_ut));